...
def model_forward(
    ...
    block_controlnet_hidden_states_npy = None,
) -> Tensor:
    if img.ndim != 3 or txt.ndim != 3:
        raise ValueError("Input img and txt tensors must have 3 dimensions.")
    
    # if block_controlnet_hidden_states_npy is not None:
    #     block_controlnet_hidden_states = None
    img = model.forward_orig(img=img, 
                             img_ids=img_ids, 
                             txt=txt, 
                             txt_ids=txt_ids, 
                             timesteps=timesteps, 
                             ...
                             block_controlnet_hidden_states_npy=block_controlnet_hidden_states_npy)
    return img
